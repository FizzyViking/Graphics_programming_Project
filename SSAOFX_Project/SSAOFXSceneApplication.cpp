#include "SSAOFXSceneApplication.h"
...
SSAOFXSceneApplication::SSAOFXSceneApplication()
    : Application(1024, 1024, "SSAO FX Scene")
...
void SSAOFXSceneApplication::Initialize()
{
...
void SSAOFXSceneApplication::Update()
{
    Application::Update();
...
void SSAOFXSceneApplication::Render()
{
    Application::Render();
...
void SSAOFXSceneApplication::Cleanup()
{
    // Cleanup DearImGUI
    m_imGui.Cleanup();
...
void SSAOFXSceneApplication::InitializeCamera()
{
...
void SSAOFXSceneApplication::InitializeLights()
{
...
void SSAOFXSceneApplication::InitializeMaterials()
{
    // G-buffer material
    {
...
void SSAOFXSceneApplication::InitializeModels()
{
...
void SSAOFXSceneApplication::InitializeFramebuffers()
{
    int width, height;
...
void SSAOFXSceneApplication::InitializeRenderer()
{
    int width, height;
    GetMainWindow().GetDimensions(width, height);
...
std::shared_ptr<Material> SSAOFXSceneApplication::CreatePostFXMaterial(const char* fragmentShaderPath, std::shared_ptr<Texture2DObject> sourceTexture)
{
...
Renderer::UpdateTransformsFunction SSAOFXSceneApplication::GetFullscreenTransformFunction(std::shared_ptr<ShaderProgram> shaderProgramPtr) const
{
    // Get transform related uniform locations
    ShaderProgram::Location invViewMatrixLocation = shaderProgramPtr->GetUniformLocation("InvViewMatrix");
    ShaderProgram::Location invProjMatrixLocation = shaderProgramPtr->GetUniformLocation("InvProjMatrix");
...
void SSAOFXSceneApplication::RenderGUI()
{
    m_imGui.BeginFrame();
